...
export const useStreamChat = () => {
  ...
  // init stream chat client
  useEffect(() => {
    console.log("useStreamChat debug:", {
      user,
      tokenData,
      isLoading,
      error,
      streamApiKeyPresent: !!STREAM_API_KEY,
    });

    if (!STREAM_API_KEY) {
      console.warn("useStreamChat: missing VITE_STREAM_API_KEY");
      return;
    }
    if (!user?.id) {
      console.warn("useStreamChat: waiting for user id");
      return;
    }
    if (!tokenData?.token) {
      console.warn("useStreamChat: waiting for tokenData.token", tokenData);
      return;
    }
